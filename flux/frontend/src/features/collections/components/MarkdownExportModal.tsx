...
interface MarkdownExportModalProps {
  ...
  opts: main.ExportMarkdownOpts;
  setOpts: (opts: main.ExportMarkdownOpts) => void;
  onExport: (c: models.Collection) => void;
}

export function MarkdownExportModal({ open, onClose, collection, opts, setOpts, onExport }: MarkdownExportModalProps) {
  if (!open) return null;
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <div className="bg-card border border-border rounded-lg shadow-xl p-4 w-[320px] pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          <h3 className="text-13 font-semibold text-text mb-3">Export Markdown — {collection.name}</h3>
